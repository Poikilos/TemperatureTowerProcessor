...
class ConfigurationFrame(ttk.Frame):
    ...
    def checkSettings(self):
        ...
        global temperature
        global height
        temperature_strings = [None, None]
        rangeNames = ["minimum", "maximum"]
        for i in range(2):
            temperature_strings[i] = self.temperatureVs[i].get()
            if len(temperature_strings[i]) == 0:
                self.temperatureVs[i].set(str(temperature_range[i]))
            try:
                temperature_range[i] = int(temperature_strings[i])
            except ValueError:
                temperature_strings[i] = str(temperature_range[i])
                self.temperatureVs[i].set(str(temperature_range[i]))
        ...
        if max_temperature < temperature_range[1]:
            self.echo("INFO: Only {} floors will be present since the"
                      ...
                      " (counting by {} C).".format(len(temperatures),
                                                    len(heights),
                                                    temperature_step))
        ...
        if len(sys.argv) < 2:
            print("")
            self.templateGcodePath.set(default_path)
            print("Checking for '{}'...".format(self.templateGcodePath.get()))
            print("")
            # return False
        else:
            self.templateGcodePath.set(sys.argv[1])
        notePath = self.templateGcodePath.get()+" is missing.txt"
        if not os.path.isfile(self.templateGcodePath.get()):
            print("")
            print("'{}' does not exist.".format(self.templateGcodePath.get()))
            if self.templateGcodePath.get() == default_path:
                notePathMsg = ""
                exMessage = ""
                try:
                    with open(notePath, 'w') as outs:
                        outs.write('You must slice the Configurable Temperature tower from {} as {}\n'.format(downloadPageURL, self.templateGcodePath.get()))
                    notePathMsg = " See 'README.md'."
                    # " + os.path.abspath(notePath) + "
                except:
                    exType, exMessage, exTraceback = sys.exc_info()
                    print(exMessage)
                    notePathMsg = " Writing '" + os.path.abspath(notePath) + "' failed: " + exMessage
                self.echo("You must first slice " + towerName + " ("
                          + downloadPageURL + ") as '"
                          + self.templateGcodePath.get() + "'" + notePathMsg)
            print("")
            return False
        else:
    ...
    def generateTower(self):
        ...
        # Start a thread so DISABLE occurs before processing.
        self.echo("* generating...")
        self.generateTimer = threading.Timer(0.01, self.generateTowerThread)
    ...
    def generateTowerThread(self):
        ...
        global stop_building
        global last_height
        global something_printed
        global previous_values
        global deltas
        global current_values
        global current_values_lines
        global given_values
        global previous_values
        global previous_dst_line
        global previous_src_line
        global level
        global start_temperature_found
        global height

        if not self.checkSettings():
            self.generateButton.config(state=tk.NORMAL)
            return False
        tmp_path = self.templateGcodePath.get() + ".tmp"
        dst_path = ("{}-{}_".format(temperature_range[0],
                                    max_temperature)
                    + os.path.splitext(self.templateGcodePath.get())[0] + ".gcode")

        with open(self.templateGcodePath.get()) as ins:
                    ...
                    if cmd_meta[0][0] == "G":
                        ...
                        if line[0:5] == stw_cmd + " ":  # set temperature & wait
                            ...
                            if start_temperature_found:
                                self.echo("Line {}: Extra temperature command at {}: {}".format(line_number, height, line)
                                          + "\n- changed to: {}".format(new_line))
                            else:
                                self.echo("Line {}: Initial temperature command at {}: {}".format(line_number, height, line)
                                          + "\n- changed to: {}".format(new_line))
                            outs.write(new_line + "\n")
                            previous_dst_line = line
                            # if level == 0:
